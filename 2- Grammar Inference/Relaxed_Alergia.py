...
def Relaxed_Alergia(sequences, alpha):
    ...
    while(len(blue) > 0):
        ...
        if(qb.get_frequency() >= t0):
            for qr in red:
                if(Relaxed_AlergiaCompatible(pfa, qr, qb, alpha)):
                    #print('Merge accepted...')
                    #print(qr)
                    #print(qb)
                    pfa = Relaxed_MergeFold(pfa, qr, qb)
                    promote = False
...
def Relaxed_AlergiaCompatible(pfa, qr, qb, alpha):
    '''
    ...
    '''
    correct = True
    if(Relaxed_AlergiaTest(qr.children[0][1], qr.get_frequency(), qb.children[0][1], qb.get_frequency(), alpha) == False):
        correct = False

    return correct

def Relaxed_AlergiaTest(f1, n1, f2, n2, alpha):
    '''
...
def Relaxed_MergeFold(pfa, qr, qb):
    '''
    MERGE OPERATION:
