def viterbi_path(self, observations, prior = None, scaled=True, ret_loglik=False):
    ...
    Outputs:
        path: np.array(num_obs)
            path[t] := Q[t]
    '''
    obslik = np.array([self.matrice_emission.to_numpy()[:, z + 1] for z in observations]).T
    num_hid = obslik.shape[0] # number of hidden states
    num_obs = obslik.shape[1] # number of observations (not observation *states*)
    
    prior = prior if prior is not None else np.full(self.matrice_transition.to_numpy().shape[0], 1 / self.matrice_transition.to_numpy().shape[0])
